...
(function ($) {
   ...
   *===================================*/
    ...
    *===================================*/
    ...
    $(document).on('click', '.ecommerce-products-tab .nav-tabs .nav-link:not([data-loaded])', function (e) {
        e.preventDefault()
        const $this = $(e.currentTarget)
        const $container = $this.closest('.ecommerce-products-tab')
        const tabPanel = $container.find('#' + $this.data('ref'))
        const $template = $container.find('.ecommerce-product-items').html()
        ...
        $.ajax({
            ...
            dataType: 'json',
            beforeSend: function() {
                tabPanel.find('.half-circle-spinner').show()
            },
            success: (res) => {
                if (res.error === false && res.data) {
                    const productItems = res.data.reduce((html, item) => {
                        return html + '<div class="item">' + item + '</div>'
                    }, '')

                    tabPanel.html($template.replace('__data__', productItems))
                    ...
                    tabPanel.html('<div class="alert alert-warning">No products found.</div>')
                }
            },
            error: function() {
                tabPanel.html('<div class="alert alert-danger">Failed to load products. Please try again.</div>')
            }
        })
    })

    /*===================================*
    QUICK VIEW - show modal with loading state before AJAX completes
    *===================================*/
    $(document).off('click', '[data-bb-toggle="quick-view-product"]').on('click', '[data-bb-toggle="quick-view-product"]', function (e) {
        e.preventDefault();
        ...
        var $modal = $('[data-bb-toggle="quick-view-modal"]');
        var $body = $modal.find('.modal-body');

        $body.html('<div class="d-flex align-items-center justify-content-center" style="min-height:400px"><div class="mfp-preloader"></div></div>');
        $modal.modal('show');
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.error) {
                    $body.html('<div class="text-center py-3"><p>' + (res.message || 'Product not available.') + '</p></div>');
                } else {
                    $body.html(res.data);

                    document.dispatchEvent(new CustomEvent('ecommerce.quick-view.initialized', {
                        detail: { data: res.data, element: $btn }
                    }));
                }
            },
            error: function () {
                $body.html('<div class="text-center py-3"><p>Failed to load product. Please try again.</p></div>');
            }
        });
    });
